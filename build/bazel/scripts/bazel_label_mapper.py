...
_IGNORED_LABELS = {
    ...
    # Files generated by repository rules for rules_cc.
    "@@rules_cc++compatibility_proxy+cc_compatibility_proxy//:BUILD",
    ...
    # Files generated by repository rules for rules_java.
    "@@rules_java++compatibility_proxy+compatibility_proxy//:BUILD",
    "@@rules_java++compatibility_proxy+compatibility_proxy//:BUILD.bazel",
    "@@rules_java++compatibility_proxy+compatibility_proxy//:proxy.bzl",
...
class BazelLabelMapper(object):
    ...
    def __init__(
        ...
        output_dir: str,
        root_repo_mapping: (
            bazel_repository_utils.BazelRootRepoMapping | None
        ) = None,
    ):
        ...
        # Root workspace repository mapping.
        if root_repo_mapping is None:
            root_repo_mapping = (
                bazel_repository_utils.BazelRootRepoMapping.new_from_build_dir(
                    Path(self._output_dir)
                )
            )

        self._root_repo_mapping = root_repo_mapping
    ...
    def _get_canonical_repo_name(self, repo_name: str) -> str:
        """Convert an apparent repository name to its canonical name.

        If the repository is present in the root repository mapping, its canonical
        name is returned. Otherwise, returns repo_name as-is (e.g. if it is already
        a canonical name or from a transitive sub-repository).
        """
        canonical = self._root_repo_mapping.get_canonical_name(repo_name)
    ...
    def _get_repository_content_hash(self, repository_name: str) -> str:
        ...
        Args:
            repository_name: Bazel repository name, can be apparent or
              canonical, and can start with @ or @@, or not.
        ...
        if hash_file is None:
            ...
            def cache_file_prefix(repo_name: str) -> str:
                """Compute cache file prefix for a given repository name.

                Args:
                    repo_name: A repository name.
                Returns:
                    A file prefix for the corresponding cache file, or
                    an empty string. Apparent and canonical names of the
                    same repository should return the same value.
                """
                raw_name = repo_name.removeprefix("@@").removeprefix("@")

                # First, if it is a known apparent name, return it as-is
                if raw_name in self._root_repo_mapping:
                    return raw_name

                # Second, try to see if it is a known canonical name, and
                # return its apparent_name
                apparent_name = self._root_repo_mapping.get_apparent_name(
                    raw_name
                )
                if apparent_name:
                    return apparent_name

                # This is likely a cnonical name for a transitive repository
                # (i.e. one not defined in the top-level MODULE.bazel) file.
                # In all cases, return an empty string as the Fuchsia build
                # does not generate content-hash files for these.
                return ""

            file_prefix = cache_file_prefix(repository_name)
    ...
    def source_label_to_path(
        ...
        Args:
          label: A fully formed Bazel label, as returned by a query. If BzlMod is
              enabled, this expects canonical repository names to be present
              (e.g. '@foo.12//src/lib:foo.cc' and not '@foo//src/lib:foo.cc').
        ...
        if repository == "" or repository == "@" or repository == "@@":
            ...
            #
            # Use the root repository mapping (or fallback to raw repo name for transitive
            # or already canonical names) to determine the canonical repository directory.
            #
            assert repository.startswith(
                "@"
            ), f"Invalid repository name in source label {label}"

            raw_repo_name = repository.removeprefix("@@").removeprefix("@")
            canonical_repo_name = self._get_canonical_repo_name(raw_repo_name)
            repository_dir = self._external_dir_prefix + canonical_repo_name
