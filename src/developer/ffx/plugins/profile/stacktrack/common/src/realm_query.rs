...
const COLLECTOR_CAPABILITY: &str = fstacktrack_client::CollectorMarker::PROTOCOL_NAME;
...
pub async fn connect_to_collector(
...
) -> anyhow::Result<fstacktrack_client::CollectorProxy> {
    ...
    let (proxy, server) =
        remote_control.domain().create_proxy::<fstacktrack_client::CollectorMarker>();
    remote_control
        .connect_capability(
            &moniker,
            OpenDirType::ExposedDir,
            fstacktrack_client::CollectorMarker::PROTOCOL_NAME,
