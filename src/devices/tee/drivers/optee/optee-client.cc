...
zx::result<fidl::ClientEnd<fio::Directory>> OpteeClient::GetStorageDirectory(
  ...
  zx::result storage = RecursivelyWalkDirectories(
      root.value(), path, fio::kPermReadable | fio::kPermWritable | fio::Flags::kProtocolDirectory);
  if (storage.is_error()) {
...
zx::result<fidl::ClientEnd<fio::Directory>> OpteeClient::CreateStorageDirectory(
    const std::filesystem::path& path) {
  auto root = GetRootStorage();
  if (root.is_error()) {
    return root.take_error();
  ...
  zx::result storage =
      RecursivelyWalkDirectories(root.value(), path,
                                 fio::kPermReadable | fio::kPermWritable |
                                     fio::Flags::kProtocolDirectory | fio::Flags::kFlagMaybeCreate);
  if (storage.is_error()) {
    return storage.take_error();
  }
  return zx::ok(std::move(storage.value()));
...
zx_status_t OpteeClient::HandleRpcCommandFileSystemOpenFile(OpenFileFileSystemRpcMessage* message) {
  ...
  auto storage_dir = GetStorageDirectory(path.parent_path());
...
zx_status_t OpteeClient::HandleRpcCommandFileSystemCreateFile(
  ...
  auto storage_dir = CreateStorageDirectory(path.parent_path());
  if (storage_dir.is_error()) {
    message->set_return_code(TEEC_ERROR_BAD_STATE);
    return storage_dir.status_value();
  }
...
zx_status_t OpteeClient::HandleRpcCommandFileSystemRenameFile(
  ...
  auto new_storage = GetStorageDirectory(new_path.parent_path());
  ...
  auto old_storage = GetStorageDirectory(old_path.parent_path());
